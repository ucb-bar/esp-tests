...
{
    asm volatile ("vsetcfg %0" : : "r" (VCFG(0, 32, 0, 16)));

    int vlen_result;
    asm volatile ("vsetvl %0, %1" : "=r" (vlen_result) : "r" (n));

    void * vpset_vfblockaddr;
    asm volatile ("la %0, sgemm_opt_v_8_8_vpset" : "=r" (vpset_vfblockaddr));
    ...
    asm volatile ("la %0, sgemm_opt_v_8_8_first"  : "=r" (first_vfblockaddr));
    asm volatile ("la %0, sgemm_opt_v_8_8_last"  : "=r" (last_vfblockaddr));
    asm volatile ("la %0, sgemm_opt_v_8_8_pre"  : "=r" (pre_vfblockaddr));
    asm volatile ("la %0, sgemm_opt_v_8_8_even" : "=r" (even_vfblockaddr));
    asm volatile ("la %0, sgemm_opt_v_8_8_odd"  : "=r" (odd_vfblockaddr));
    asm volatile ("la %0, sgemm_opt_v_8_8_post" : "=r" (post_vfblockaddr));

    asm volatile ("vmca va10, %0" : : "r" (n*sizeof(float)));
    asm volatile ("vmca va11, %0" : : "r" (n*sizeof(float)));
    asm volatile ("vmca va18, %0" : : "r" (n*sizeof(float)));
    for (int i = 0; i < n; i+=8) {
        for (int k = 0; k < n; ) {
            int consumed;
