...
{
    asm volatile ("vsetcfg %0" : : "r" (VCFG(32, 0, 0, 1)));
    ...
    asm volatile ("la %0, dgemm_opt_v_8_8_first"  : "=r" (first_vfblockaddr));
    asm volatile ("la %0, dgemm_opt_v_8_8_last"  : "=r" (last_vfblockaddr));
    asm volatile ("la %0, dgemm_opt_v_8_8_pre"  : "=r" (pre_vfblockaddr));
    asm volatile ("la %0, dgemm_opt_v_8_8_even" : "=r" (even_vfblockaddr));
    asm volatile ("la %0, dgemm_opt_v_8_8_odd"  : "=r" (odd_vfblockaddr));
    asm volatile ("la %0, dgemm_opt_v_8_8_post" : "=r" (post_vfblockaddr));

    asm volatile ("vmca va10, %0" : : "r" (n*sizeof(double)));
    asm volatile ("vmca va11, %0" : : "r" (n*sizeof(double)));
    asm volatile ("vmca va18, %0" : : "r" (n*sizeof(double)));
    for (int i = 0; i < n; i+=8) {
        for (int k = 0; k < n; ) {
            int consumed;
